...
export default class Firewall {
  constructor(options) {
    ...
    const whitelistedDomains = ['https://sourcewifi.com',
      'https://ropsten.infura.io/3hH8tgdKIa5RFvXCXRkG'];
      // Add additional? such as infura if we can't set up node
    const whitelistedPorts = ['8080', '8545'];
      //Where 8545 is the default eth node port on localhost
  ...
  getAccountingAllDevices() {
    return _.flatten(GET_ALL_ACCOUNTING_SESSIONS_COMMANDS)).forEach(execSync);
  }

  getAccountingAllDevicesIn() {
    return _.flatten(GET_INET_IN)).forEach(execSync);
  }

  getAccountingAllDevicesOut() {
    return _.flatten(GET_INET_OUT)).forEach(execSync);
  }

  getTrafficToPort(macAddress, port) {
    return _.flatten(GET_TRAFFIC_TO_PORT(macAddress, port)).forEach(execSync);
  }
